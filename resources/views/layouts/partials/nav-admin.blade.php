<header class="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 shadow-sm sticky top-0 z-50 transition-colors duration-300" x-data="{ mobileMenuOpen: false }">
        ...
        <div class="flex justify-between h-20">
                ...
                <nav class="hidden md:ml-12 md:flex space-x-1">
                    @php
                        $navItems = [
                            ...
                            ['route' => 'admin.soal', 'label' => 'Create Exam', 'active' => request()->routeIs('admin.soal')],
                            ['route' => 'admin.achievements.index', 'label' => 'Achievements', 'active' => request()->routeIs('admin.achievements.*')],
                            ['route' => 'admin.welcome.settings', 'label' => 'Home CMS', 'active' => request()->routeIs('admin.welcome.settings')],
                            ['route' => 'admin.users.index', 'label' => 'Manage Users', 'active' => request()->routeIs('admin.users.*')],
                        ];
                    @endphp
            ...
            <div class="hidden md:flex items-center gap-4">
                ...
                <a href="{{ route('profile') }}" class="text-right border-r border-slate-200 dark:border-slate-600 pr-4 hover:opacity-80 transition-opacity">
                    <p class="text-[10px] font-black text-[#00bceb] uppercase tracking-[0.15em] leading-none mb-1.5">Administrator</p>
    ...
    <div x-show="mobileMenuOpen" class="md:hidden bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 transition-colors duration-300"
        ...
        <div class="px-4 pt-4 pb-4 space-y-2">
            ...
            <button @click="$dispatch('theme-toggle')" 
                    class="w-full flex items-center justify-between px-4 py-3 rounded-xl text-sm font-black uppercase tracking-widest transition-colors duration-200 bg-slate-50 dark:bg-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-600">
                <span>Theme</span>
            ...
            @foreach($navItems as $item)
                <a href="{{ route($item['route']) }}"
                   ...
                   {{ $item['active'] ? 'bg-[#eefbff] dark:bg-slate-700 text-[#005073] dark:text-[#00bceb]' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 hover:text-[#00bceb]' }}">
                    {{ $item['label'] }}
                </a>
            @endforeach
            <div class="border-t border-slate-200 dark:border-slate-600 my-2 pt-2">
                <a href="{{ route('profile') }}" class="px-4 py-2 block hover:bg-slate-50 dark:hover:bg-slate-700 rounded-xl transition-colors">
                     <p class="text-[10px] font-black text-[#00bceb] uppercase tracking-[0.15em] mb-1">Signed in as</p>
