...
<div class="mb-8 flex justify-between items-end">
    ...
    <div class="flex gap-3">
        <a href="{{ route('admin.exams.report', $exam) }}"
           class="flex items-center gap-2 px-6 py-3 bg-[#005073] dark:bg-[#00bceb] text-white dark:text-slate-900 rounded-2xl font-bold text-xs hover:bg-[#003d58] dark:hover:bg-[#008ebb] transition shadow-lg shadow-blue-100 dark:shadow-none uppercase tracking-widest transition-colors">
...
{{-- 1. ANALYTICS CARDS --}}
<div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10 transition-colors">
    <div class="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden transition-colors">
        <div class="text-slate-400 dark:text-slate-500 text-[10px] font-black uppercase tracking-widest mb-2 transition-colors">Participants</div>
        <div class="text-3xl font-black text-[#005073] dark:text-white transition-colors">{{ $submissions->count() }}</div>
    ...
    <div class="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden border-t-4 border-t-[#E2231A] dark:border-t-rose-500 transition-colors">
        ...
        <div class="absolute -right-2 -bottom-2 opacity-10 dark:opacity-20 font-black text-6xl text-red-100 dark:text-rose-900 transition-colors transition-colors">!</div>
    </div>
</div>
...
<div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden transition-colors">
    <div class="px-8 py-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/50 transition-colors transition-colors">
    ...
    <table class="w-full text-left">
        <thead>
            <tr class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest border-b border-slate-100 dark:border-slate-700 transition-colors">
                ...
                <th class="px-8 py-4 text-center">Time Spent</th> {{-- KOLOM BARU --}}
            ...
            <tr class="hover:bg-[#eefbff]/30 transition group">
                ...
                <td class="px-8 py-5 text-center">
                    <div class="inline-flex items-center gap-1.5 px-3 py-1 bg-slate-100 dark:bg-slate-900 rounded-full text-slate-600 dark:text-slate-400 text-xs font-bold transition-colors">
                        ...
                        {{ $sub->started_at ? $sub->started_at->diffInMinutes($sub->created_at) . 'm' : '-' }}
                    </div>
                </td>
                <td class="px-8 py-5 text-center">
