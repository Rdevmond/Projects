...
<div class="max-w-7xl mx-auto px-6 py-10" x-data="{
    ...
    <div class="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            <div class="inline-flex items-center px-3 py-1.5 rounded-lg bg-[#00bceb]/10 dark:bg-[#00bceb]/20 text-[#005073] dark:text-[#00bceb] text-[10px] font-black uppercase tracking-widest mb-3 border border-[#00bceb]/30">
        ...
        <div class="relative w-full md:w-80">
            <input type="text" x-model="search" placeholder="Search name or school..."
                class="w-full pl-12 pr-4 py-4 bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl text-sm focus:ring-4 focus:ring-[#00bceb]/20 focus:border-[#00bceb] transition shadow-sm font-bold text-slate-800 dark:text-white placeholder-slate-400 dark:placeholder-slate-500">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-[3rem] shadow-2xl shadow-slate-200/60 dark:shadow-black/40 border border-slate-100 dark:border-slate-700 p-10 md:p-12 mb-10 relative overflow-hidden transition-colors duration-300">
        {{-- Decorative gradient --}}
        <div class="absolute top-0 right-0 w-96 h-96 opacity-10 dark:opacity-20 pointer-events-none" style="background: radial-gradient(circle, #00bceb 0%, transparent 70%);"></div>
        
        <div class="flex items-center gap-4 mb-8 relative z-10">
            <div class="p-3 rounded-2xl bg-linear-to-br from-[#005073] to-[#00bceb]">
            ...
            <div>
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400 font-semibold">Add a new participant to the system</p>
            </div>
        </div>
        ...
        <form action="{{ route('admin.users.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-5 relative z-10">
            ...
            <div class="lg:col-span-1 flex items-end">
                <button type="submit" class="w-full px-6 py-3 rounded-2xl font-black uppercase tracking-widest text-xs text-white shadow-lg shadow-[#E2231A]/20 dark:shadow-none hover:shadow-xl transition-all transform hover:-translate-y-0.5 active:scale-95 bg-gradient-to-br from-[#E2231A] to-[#be1e16]">
                    Add Student
                </button>
            </div>
        </form>
        ...
        <div class="p-8 md:p-10 border-b border-slate-100 dark:border-slate-700">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <div class="w-12 h-12 bg-[#005073]/5 dark:bg-[#00bceb]/10 rounded-2xl flex items-center justify-center transition-colors">
                        <svg class="w-6 h-6 text-[#005073] dark:text-[#00bceb]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                    </div>
                    <div>
                        <h3 class="text-xl font-black text-[#005073] dark:text-white tracking-tight uppercase transition-colors">Student Roster</h3>
                        <p class="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest transition-colors">Currently registered in system</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 dark:bg-slate-800/50 transition-colors">
                        <th class="px-8 py-5 text-[10px] font-black text-[#005073] dark:text-[#00bceb] uppercase tracking-widest border-b border-slate-100 dark:border-slate-800 transition-colors">Name & Credentials</th>
                        <th class="px-8 py-5 text-[10px] font-black text-[#005073] dark:text-[#00bceb] uppercase tracking-widest border-b border-slate-100 dark:border-slate-800 transition-colors">Institution</th>
                        <th class="px-8 py-5 text-[10px] font-black text-[#005073] dark:text-[#00bceb] uppercase tracking-widest border-b border-slate-100 dark:border-slate-800 text-right transition-colors">Account Actions</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800 transition-colors">
                    ...
                    <tr x-show="search === '' || '{{ strtolower($user->name) }} {{ strtolower($user->school) }}'.includes(search.toLowerCase())"
                        class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition group transition-colors">
                        <td class="px-8 py-6">
                            <div class="flex items-center gap-4">
                                <div class="w-10 h-10 rounded-full bg-linear-to-br from-[#005073] to-[#00bceb] flex items-center justify-center text-white font-black text-xs shadow-md">
                                    {{ substr($user->name, 0, 1) }}
                                </div>
                                <div>
                                    <div class="font-black text-slate-800 dark:text-white group-hover:text-[#005073] dark:group-hover:text-[#00bceb] transition-colors transition-colors">{{ $user->name }}</div>
                                    <div class="text-xs text-slate-400 dark:text-slate-500 font-bold transition-colors">{{ $user->username }} • {{ $user->email }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-8 py-6">
                            <span class="px-3 py-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 text-[10px] font-black uppercase tracking-widest rounded-lg border border-slate-200 dark:border-slate-700 transition-colors">
                                {{ $user->school ?? 'N/A' }}
                            </span>
                        </td>
                        <td class="px-8 py-6 text-right">
                            <div class="flex justify-end items-center gap-2">
                                <a href="{{ route('admin.users.edit', $user) }}" 
                                   class="p-2.5 bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-600 hover:text-[#00bceb] dark:hover:text-[#00bceb] hover:bg-[#00bceb]/10 rounded-xl transition-all transition-colors" title="Edit Student">
                                ...
                                <button type="button" @click="confirmDelete('{{ route('admin.users.destroy', $user) }}', '{{ $user->name }}')"
                                    class="p-2.5 bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-600 hover:text-rose-500 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-xl transition-all transition-colors" title="Delete Student">
                    ...
                    <tr>
                        <td colspan="3" class="px-8 py-20 text-center">
                            <div class="flex flex-col items-center gap-4">
                                <div class="w-20 h-20 bg-slate-50 dark:bg-slate-800 rounded-[2rem] flex items-center justify-center transition-colors">
                                ...
                                <span class="text-slate-400 dark:text-slate-600 font-bold italic transition-colors transition-colors">No students registered yet. Populate your roster.</span>
                            </div>
                        </td>
                    </tr>
                    @endforelse
