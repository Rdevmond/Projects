...
<div x-data="examBuilder({{ $exam->questions->toJson() }}, {{ $allStudents->toJson() }}, {{ json_encode($assignedUserIds) }}, '{{ $exam->exam_mode ?? 'normal' }}', '{{ $exam->duration_mode ?? 'global' }}')" class="max-w-5xl mx-auto py-12 px-4 transition-colors" x-cloak>
 ...
 <div class="bg-slate-50/95 dark:bg-slate-950/95 backdrop-blur-md border-2 border-slate-200 dark:border-slate-800 border-b-8 border-b-[#E2231A] dark:border-b-[#00bceb] rounded-full p-4 flex items-center justify-between shadow-[0_20px_50px_-12px_rgba(0,0,0,0.3)] dark:shadow-[0_20px_50px_-12px_rgba(0,0,0,0.7)] transition-all duration-300">
 ...
 <button type="submit" class="bg-[#E2231A] dark:bg-[#00bceb] hover:bg-[#c91811] dark:hover:bg-[#00a3cc] text-white dark:text-slate-900 px-10 py-4 rounded-full font-black uppercase tracking-widest text-xs shadow-xl transition-all active:scale-95">
 Update Exam Data
 </button>
 </div>
 </div>
